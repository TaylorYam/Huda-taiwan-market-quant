...
「最早可查日期」、「查詢頁目前可回溯的窗口」和「本次實際量到的缺值率」是不同欄位。本次未下載每個來源的完整日序列與交易日曆，因此缺值率一律標為未測，不以目前 API 有回資料推定全期間完整。TAIFEX PCR 最新抽樣資料日為 2026-09-16；VIX 日收盤月檔已直接核實 2026-09-15 日列。
...
| TXO OI Put/Call Ratio | TAIFEX | 已核驗日級 CSV 含 2001-12-24 起始列 | 2026-09-16 | 日 | 未測 | 單次起訖日期差最多 30 日；OpenAPI 為最新快照，長期完整覆蓋率未測 |
| Taiwan VIX（日收盤） | TAIFEX | 免費頁當月與前三個完整月份、指數查詢近 3 年；付費歷史商品自 2007 年 | 2026-09-15 | 日 | 未測 | 月檔 tab-separated 純文字可累積；任意日期 endpoint 未知；付費商品 NT$3,000／半年 |
...
| TXO OI Put/Call Ratio | TAIFEX [Put/Call Ratio 官方頁](https://www.taifex.com.tw/cht/3/dlPcRatio) 網頁表單 POST 支援日期區間；下載端點為 `/cht/3/pcRatioDown`，欄位為日期、買／賣權成交量及比率、Put／Call OI 及比率；OpenAPI `GET /PutCallRatio` 無日期參數，只回最新快照。 | 最早可取列 2001-12-24；最新抽樣資料日 2026-09-16。UI 起訖相差最多 30 個曆日、端點含在內（每段最多 31 日），全期估計 292 個不重疊區段；只抽查代表區段，連續完整度未驗證。 | 每交易日；官方頁合併週到期與月契約。回補應按非重疊日曆區段執行，日期唯一；缺列保留為無資料，不補 0。 | 抽查 2001 上市初期、2025 端午補假週及最新區段，資料列無重複；完整下載後仍需以交易日曆驗證覆蓋率，CSV 編碼尚未由官方文件明載。 |
| Taiwan VIX（日收盤） | 免費[每日收盤頁](https://www.taifex.com.tw/cht/7/vixDaily3MNew)可見 2026/06–09 月；月檔例如 [`202609new.txt`](https://www.taifex.com.tw/file/taifex/Dailydownload/vix/log2data/202609new.txt) 為 tab-separated 純文字。`20260915` 日列為 VIX 27.29、收盤前 1 分鐘平均 27.29；[指數專區](https://www.taifex.com.tw/indes/index.aspx)「3年」圖表窗口為 2023-09-16–2026-09-15。 | 免費日收盤頁：當月與前三個完整月份；圖表查詢：最近三年。付費歷史商品頁列示起自 2007-01-01、至申購日前一完整月份，NT$3,000／半年。任意日期下載 endpoint 與 CSV 格式未知。 | 日級欄位為交易日期、時間、VIX、收盤前 1 分鐘平均；月檔可按月份累積後自行篩選日期。 | 日收盤月檔與單日 payload 已核實，但完整日序列缺值率未測；圖表尚未驗證可匯出日級資料。付費商品不代表已取得資料。 |
