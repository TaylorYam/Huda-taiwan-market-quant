...
3. **定義資料契約及保存方式。** [`data-contract-v0.1.md`](data-contract-v0.1.md) 已建立邏輯欄位、日期時區、單位、契約、盤別、發布時間、來源、修訂與缺值語義；[ADR 0001](adr/0001-phase-1-storage-boundary.md) 已確認 Phase 1 單機開發使用 Git 忽略的 SQLite，介面與 schema 見 [Storage Interface](storage-interface-v0.1.md)。免費 MVP 的正式持久來源、部署層與升級邊界已由 [ADR 0002](adr/0002-free-tier-mvp-stack.md) 選定，正式每日流程仍須完成備份、還原、權限與配額驗證。
