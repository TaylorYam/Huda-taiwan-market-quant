...
00:30（分別為 `30 8`、`0 14` 與 `30 16` UTC，星期範圍均為 UTC 週一至週五）；
...
| 外資現貨現金 | `twse_foreign_cash_bfi82u_v1`；TWSE BFI82U 日報 | `.github/workflows/twse-free-factor-daily-ingestion.yml`；手動入口。每日排程由整合流程執行 | `date` 可指定 `YYYY-MM-DD`；省略取當日 Asia/Taipei。回補 workflow 仍使用 `start`、`end`、`write` | 週末／台灣假日沒有日列；BFI82U 有發布版本／時間差，整合流程於 16:30、22:00、隔日 00:30 確認 | 日列 label 為 `day:YYYY-MM-DD`，相同 payload 可去重；單一整合流程避免獨立排程競爭 |
...
| TXO PCR／OI PCR | `taifex_txo_oi_pcr_v1`；TAIFEX PCR 日期表單 | `.github/workflows/taifex-pcr-daily-ingestion.yml`；手動入口。每日排程由整合流程執行 | `date`；省略時取當日 Asia/Taipei。`collect_taifex_pcr.py --dry-run` 可只驗證 | 台灣假日（即使是平日）沒有該日期資料，parser 會失敗；整合流程會在 16:30、22:00、隔日 00:30 依序重試 | label 為 `day:YYYY-MM-DD`；相同 payload 去重，內容修訂追加 revision |
