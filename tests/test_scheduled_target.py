...
def test_delayed_first_pass_after_midnight_keeps_previous_market_date() -> None:
    ...
    assert resolve_scheduled_target_date(now=now, schedule="30 8 * * 1-5") == datetime(
        2026, 9, 21, tzinfo=TAIPEI
    ).date()
...
def test_confirmation_pass_keeps_same_date_after_planned_time() -> None:
    ...
    assert resolve_scheduled_target_date(now=now, schedule="0 14 * * 1-5") == datetime(
        2026, 9, 21, tzinfo=TAIPEI
    ).date()
