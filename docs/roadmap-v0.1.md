...
2. **補完歷史窗口探測。** PCR 日期表單已完成 292 段全期稽核，回傳 6,090 個唯一日期；仍需用完整交易日曆解釋無資料日。TX 年檔 1998–2025 已完成 28 份全檔稽核，合計 84,828 筆 TX 列、6,819 個年度日期、0 錯誤與 0 重複鍵；1998 官方交易日曆與 2026 年度檔仍未知。VIX 日收盤月檔與 2026-09-15 日列已核實，免費窗口為當月與前三個完整月份；任意日期 endpoint 與三年圖表匯出仍未知。法人 OI 公開頁僅能查近三年，舊資料走申請路徑。詳見[TAIFEX 歷史窗口補查](phase0-taifex-history-v0.1.md)。
...
- **程式狀態（2026-09-17 更新）：** 已有 Phase 1 SQLite observation interface、schema、冪等與 revision 測試，TAIEX、VIX、PCR、TX、外資現貨（BFI82U／FMTQIK）與法人期貨 OI 的官方日級入口與一次性歷史回補，以及 Supabase Data API 寫入與 CI quality/tests 分線。8 個核心因子與 Market Score 已實作（`src/factors`、`src/scoring`），含以 `build_historical_values` 現算的 point-in-time 歷史百分位；已用合成資料端對端驗證全部 8 個因子可計分。第 11 項第一層回測引擎已實作於 `src/backtest/layer1.py`（見 [`backtest-spec-v0.1.md`](backtest-spec-v0.1.md) 的實作現況段落），但尚未有足夠真實歷史資料可產出正式驗證結論。Dashboard 尚未實作。
