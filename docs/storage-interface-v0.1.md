...
`Observation` 對應 [Data Contract v0.1](data-contract-v0.1.md) 的共同欄位：資料集、交易日、來源日期／URL／列鍵、發布與有效時間、擷取／寫入時間、來源版次、payload SHA-256、parser 版本、JSON `values`、品質狀態與稽核備註。`values` 保持來源單位，空值由 `quality_status` 與 `quality_notes` 說明，不補成 0 或中性分數。

時間驗證規則為：`observation_date`／`source_date` 使用 `YYYY-MM-DD`；`retrieved_at`／`ingested_at` 必須含時區且為 UTC；其他來源時間若存在也必須含時區。
...
- 同一 identity 與相同 payload hash 重跑時回傳既有 ID，增加 `retrieval_count` 並更新 `last_retrieved_at`。
