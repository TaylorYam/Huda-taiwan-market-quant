...
- **Phase 0 進度：** Issue #5 的 BFI82U／FMTQIK 口徑查證已完成，外資現貨因子可從 2010-01-01 起計算；Issue #7 的 PCR 與 TX 年檔稽核已完成，VIX／法人 OI／TX 2026 日級資料與免費因子資料已實際回補。展示回放區間 `2023-09-25..2026-09-18` 已有 724/725 天可用 Market Score；剩餘一天是免費法人 OI 窗口邊界，三年百分位的最早暖機日為 2026-09-25，因此正式 Layer 1 回測尚未就緒。
- **程式狀態（2026-09-21 更新）：** 已有 SQLite observation interface、schema、冪等與 revision 測試，TAIEX、VIX、PCR、TX、外資現貨與法人期貨 OI 的官方日級入口與回補工具，以及 Supabase Data API 寫入。8 個核心因子、Market Score、point-in-time 歷史百分位、展示用歷史回放、Layer 1 回測引擎與 Streamlit 唯讀 Dashboard 均已實作；回測仍缺足量真實歷史資料以產出正式辨識力結論。
- **下一個工作包：** 以 Issue #77 的每日資料與 Market Score 自動化為最高優先級，先把 TAIEX、外資現貨／成交值、TAIFEX TX、法人期貨 OI、PCR、VIX 與評分串成可監督、可重跑的單一每日流程；展示版維持唯讀並同步驗收更新結果。先完成連續交易日的排程穩定性與失敗摘要，再開放正式無人值守。#18 的備份／還原、權限分離、配額與來源條款驗證排到最後。資料窗口跨過三年百分位暖機、且 forward window 有足夠後續日期後，才產出 Layer 1 真實資料報告。
