"""Fetch one verified TWSE TAIEX month and persist it through Supabase."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fetch a TWSE TAIEX month and write daily observations."
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.dry_run:
            observations = fetch_taiex_month(year, month)
            print(
                f"TAIEX {year:04d}-{month:02d}: "
        ...
        with SupabaseRestObservationStore(project_url, secret_key) as store:
            results = collect_taiex_month(store, year, month)
        summary = summarize_write_results(results)
        counts = ", ".join(f"{action}={count}" for action, count in summary.items())
        print(f"TAIEX {year:04d}-{month:02d}: {len(results)} daily rows written ({counts})")
        return 0
    except Exception as exc:
        print(f"TAIEX ingestion failed: {exc}")
        return 1
