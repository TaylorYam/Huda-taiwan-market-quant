"""Fetch one day's TWSE BFI82U foreign cash net buy/sell and persist it."""
...
from src.data.twse_foreign_cash import collect_foreign_cash_day, fetch_foreign_cash_day
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fetch one TWSE BFI82U day and write its observation."
    )
    parser.add_argument(
        "--date",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.dry_run:
            observations = fetch_foreign_cash_day(target)
            print(
                f"TWSE foreign cash {target.isoformat()}: "
                f"net_buy_sell={observations[0].values['net_buy_sell']} "
                f"category={observations[0].values['category']!r} "
                "(read-only)"
            )
            return 0
        ...
        with SupabaseRestObservationStore(project_url, secret_key) as store:
            results = collect_foreign_cash_day(store, target)
        print(
            f"TWSE foreign cash {target.isoformat()}: "
        ...
        print(f"TWSE foreign cash ingestion failed: {exc}")
        return 1
