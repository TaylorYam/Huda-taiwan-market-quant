...
欄位、時間戳、單位、修訂及缺值的共同語義見 [`data-contract-v0.1.md`](data-contract-v0.1.md)。Phase 1 本機開發儲存方案的比較與建議見 [`data-storage-options-v0.1.md`](data-storage-options-v0.1.md)，實作邊界見 [`storage-interface-v0.1.md`](storage-interface-v0.1.md)；正式排程的持久來源、格式與保留期仍須經 Accepted ADR 決定。
