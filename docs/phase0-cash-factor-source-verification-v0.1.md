...
**2026-09-17 追加查證：找到鉅額口徑轉換的大致區間，因子閘門部分解除。** 新增樣本 2008-06-05（自營商／投信／外資／合計，簡單四欄命名）與 2015-06-15（自營商(自行買賣)／自營商(避險)／投信／外資及陸資／合計，五欄命名，尚無獨立外資自營商列）：兩者的報表註記皆為「本統計資訊含一般、零股、盤後定價、鉅額，不含拍賣、標購」，與 FMTQIK 及 2022-10-24、2026-09-14 的口徑一致。也就是說，2004-04-07（不含鉅額）與 2008-06-05（含鉅額）之間發生過一次口徑轉換，轉換後到 2026-09-14 為止的 18 年樣本點皆為含鉅額。另外實測 BFI82U／FMTQIK 的 `response=csv` 端點可直接取得乾淨的 CP950 CSV（先前查證環境回報的「無法取得可讀 bytes」是該次環境或工具限制，非官方伺服器問題）。

基於這個發現，`src/data/twse_foreign_cash.py` 將因子的可用起點設為 `FOREIGN_CASH_VERIFIED_START = 2010-01-01`——在已確認的 2008-06-05 樣本之後留一段緩衝，早於此日期的觀察不會被收集或視為可用。這**沒有解決**確切轉換日期、2004 年前後的分類定義映射，或 E-Shop 14:50／19:40 版次對應；這些仍是未解問題，只是不再阻擋 2010 年以後的即時因子計算。詳見 [`docs/factor-model-v0.1.md`](factor-model-v0.1.md) 5.3 節與 [`data-window-policy-v0.1.md`](data-window-policy-v0.1.md)（模型實際共同起點仍由 VIX／法人期貨 OI 決定，現貨因子的 2010 起點不是目前的瓶頸）。
...
**判定（2026-09-17 更新）：2010-01-01 起可用；2010 年以前仍阻擋在版本定義。** 固定使用免費報表（不使用 E-Shop），分子取 BFI82U 的外資（及陸資）淨買賣金額列、不含外資自營商，分母取 FMTQIK 市場成交金額；`FOREIGN_CASH_VERIFIED_START = 2010-01-01` 之前的資料仍不視為可用觀察。剩餘未解項目（僅影響 2010 年以前的資料，不影響目前的即時因子計算）：

1. ~~固定分子要用免費報表、E-Shop BFI82U 或 E-Shop BFIBGU~~——已定案採免費報表，不使用 E-Shop，故 E-Shop 檔案碼與 14:50／19:40 版次對應問題不再阻擋此因子。
2. 2004-04-07 至 2008-06-05 之間鉅額交易口徑轉換的確切日期仍未定位；此區間內的資料仍不可用。
3. 2004 年「外資」單欄與現行「外資及陸資（不含外資自營商）」＋「外資自營商」拆列之間的逐日對應關係，2010 年以前若要延伸使用仍需另行核對。
4. 以合法可取得的原始 CSV 樣本核實跨年份的欄位、編碼與缺值率；目前只核對了 2008、2015、2022、2026 四個抽樣日期，不是逐日驗證。
5. 另行確認自動化收集、保存與展示的權利範圍——已併入 [Issue #18 的 source terms 查核](issue-18-operations-runbook-v0.1.md)，owner 已取得 TWSE／TAIFEX 書面確認（僅限非商業用途＋標示來源），此項目視為與其他來源一致處理，不再是本因子獨有的未解問題。
