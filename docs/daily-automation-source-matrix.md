...
`daily-market-automation.yml`，使用 16:30 與 22:00 Asia/Taipei，以及隔日
00:30、06:30（16:30、22:00 使用 `30 8` 與 `0 14` UTC 週一至週五；
00:30 使用 `30 16 * * 1` 至 `30 16 * * 5`，06:30 使用 `30 22 * * 1` 至
`30 22 * * 5`，兩組分別對應台北週二至週六）；兩組隔日班次按官方 TWSE
開休市日曆回推前一交易日，日曆暫時性網路錯誤會有限次重試，仍無法驗證時安全停止；
各來源 workflow 保留手動入口，不再各自排程。
...
| 外資現貨現金 | `twse_foreign_cash_bfi82u_v1`；TWSE BFI82U 日報 | `.github/workflows/twse-free-factor-daily-ingestion.yml`；手動入口。每日排程由整合流程執行 | `date` 可指定 `YYYY-MM-DD`；省略取當日 Asia/Taipei。回補 workflow 仍使用 `start`、`end`、`write` | 週末／台灣假日沒有日列；00:30、06:30 以前一個 TWSE 交易日為目標並以來源日期守門 | 日列 label 為 `day:YYYY-MM-DD`，相同 payload 可去重；單一整合流程避免獨立排程競爭 |
...
| 外資台指期 OI | `taifex_institutional_futures_oi_v1`；TAIFEX OpenAPI latest snapshot | `.github/workflows/taifex-institutional-daily-ingestion.yml` **只有手動 dispatch**，沒有 schedule。`write` 必填但預設 `false` | `write=true` 時必須同時提供 `expected_date`；讀取驗證可省略日期。workflow 會把 `--expected-date` 傳給 collector；另有 `taifex-range-backfill.yml` 的 `start`、`end`、`write` | 假日或發布延遲仍可能回傳上一交易日，但 guarded write 會在日期不符時於寫入前失敗，不保存 stale snapshot。OpenAPI 是最新快照，不是日期查詢 | latest collector label 為 `latest`；rolling-range backfill label 為 `day:YYYY-MM-DD`。兩條入口仍可能形成不同 logical rows，需避免同時執行 |
| TXO PCR／OI PCR | `taifex_txo_oi_pcr_v1`；TAIFEX PCR 日期表單 | `.github/workflows/taifex-pcr-daily-ingestion.yml`；手動入口。每日排程由整合流程執行 | `date`；省略時取當日 Asia/Taipei。`collect_taifex_pcr.py --dry-run` 可只驗證 | 00:30、06:30 班次先以 TWSE 官方日曆選出前一交易日；來源仍須回傳該日資料 | label 為 `day:YYYY-MM-DD`；相同 payload 去重，內容修訂追加 revision |
