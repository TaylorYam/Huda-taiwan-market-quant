...
| TX 行情（Basis） | TAIFEX | 官方年檔選單列 1998–2025；全檔稽核 28 份 ZIP，84,828 筆 TX 列、6,819 個年度日期、0 錯誤、0 重複鍵；2026 年由日期查詢頁補回 172 個交易日 | 2026-09-17（日期查詢頁） | 日／分盤別 | 每年度 TX 日期都有正成交量列；1998–2016 無盤別，2017 起 schema 轉換，2018–2025 含一般／盤後；2026 回補只保存日盤「一般」 | 1998 年 TAIFEX 官方日曆仍未找到；TWSE 史料可解釋當年星期六交易背景但不能取代 TX 日曆；2026 年度檔尚未列出；價格與 Basis 仍需固定近月及轉倉規則 |
...
| 外資 TX 未平倉淨部位（及 5 日變化） | TAIFEX [三大法人區分期貨契約資料](https://www.taifex.com.tw/cht/3/futContractsDateView)；[OpenAPI 規格](https://openapi.taifex.com.tw/) 的 `MarketDataOfMajorInstitutionalTradersDetailsOfFuturesContractsBytheDate` 可取得最新快照。2026-09-17 實測 JSON 最新快照為 2026-09-16；以契約 `臺股期貨`、法人 `外資及陸資` 取 `OpenInterest(Net)`，該列為 -76,351 口。 | 官方頁標示資料自 2008-04-07 起；一般歷史查詢僅提供查詢日前三年，較早資料需填公開資料申購表。OpenAPI 端點沒有日期查詢參數，只回最新快照。 | 每交易日。欄位含 `Date`、契約、法人、買／賣／淨交易量與買／賣／淨未平倉口數；新的 `taifex_institutional` collector 會保存來源日期、抓取時間與修訂 lineage，並可由六筆日列計算 5 日變化。OpenAPI 適合每日增量保存；直接 API 不適合回填。 | 每日 13:45 公布前的資料不完整；16:15 最後一批才含國外成分證券 ETF／境外指數 ETF 交易量。回測須選定一致的公布版本與 as-of 時間。最後結算日未平倉量自 2008-12-17 起不含當月份到期商品。端點目前只回最新快照，歷史回補仍未解決。 |
| TX 期貨行情（Basis） | TAIFEX [期貨每日行情查詢](https://www.taifex.com.tw/cht/3/futDailyMarketReport?commodityId=TX)、[年度行情下載](https://www.taifex.com.tw/cht/3/dlFutDailyMarketView) 及 OpenAPI `DailyMarketReportFut`。年度 ZIP 表單 POST `/cht/3/futDataDown`（`down_type=2`、`his_year`）；選單列 1998–2025 共 28 年；[全期稽核 run 35186014961](https://github.com/TaylorYam/Huda-taiwan-market-quant/actions/runs/35186014961) 已逐年完成；2026 使用日期查詢頁 POST 回補。 | 1998–2025 全檔稽核：84,828 筆 TX 列、6,819 個年度日期；2026 日期頁至 2026-09-17 寫入 172 個交易日。 | CP950 CSV；1998–2014 資料列 16 欄，2015–16 出現 16／17 欄，2017 為 18／19 欄，2018–2025 為 20 欄加一筆單欄終止列。2017 起保留交易時段，近年一般／盤後資料應用盤別納入主鍵；TX 價格與 Basis 需固定近月及轉倉規則。2026 日期頁 parser 只取日盤「一般」。 | 28 年均無年度內重複鍵，且每個年度 TX 日期都有正成交量列；2024/25 日期集合與官方行事曆及臨時休市公告相符；覆蓋率報告 run 35299636185 確認 TX 為 729 日期（2023-09-18..2026-09-17）。1998 歷史官方交易日曆未取得，不能判定逐日缺漏。見[TAIFEX 歷史窗口補查](phase0-taifex-history-v0.1.md)。 |
